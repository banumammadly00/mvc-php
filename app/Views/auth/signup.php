          ...
          <div class="row">
            ...
            <div class="col-lg-6 bg-white">
              <div class="form d-flex align-items-center">
                <div class="content">
                  <form action="/mvc-test/auth/_signup" method="post">

                    <?php  if(isset($user_error)) { ?>
                    <div style="background-color: #722c2c; margin-bottom: 15px;">
                        <span style="font-size: medium;"  ><b></b><?php echo $user_error; ?></span>
                    </div>
                    <?php } ?>

                    <?php  if(isset($form_error)){ ?>
                    <?php  foreach($form_error as $values) { ?>
                    <?php  foreach($values as $key=>$value) { ?>
                    <div style="background-color: #722c2c; margin-bottom: 15px;">
                    <span style="font-size: medium;"  ><b> </b><?php print_r( $value );  ?></span>
                    </div>
                    <?php }  } } ?>
                    ...
                    <div class="form-group-material">
                      <input id="register-password" type="password" name="confirm-pass" required data-msg="Please enter your password" class="input-material">
                      <label for="register-password" class="label-material">Confirm Password </label>
                    </div>
                    <div class="form-group text-center">
                      <input id="register" type="submit" name="submit" value="Sign up" class="btn btn-primary">
                    </div>
                  </form><small>Already have an account? </small><a href="<?php echo SITE_URL ?>/auth/login" class="signup">Login</a>
                </div>
              </div>
            </div>
          </div>
